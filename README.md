...
BiliHUD 的设置（直播间号、镜像、OBS 推流密码等）保存在 JSON 配置文件中，路径遵循 XDG 规范：`~/.config/bilihud/config.json`（可通过 `XDG_CONFIG_HOME` 环境变量覆盖）。

* **原子写入**：保存时先写入临时文件再替换，避免写入中途崩溃导致配置损坏。
* **权限保护**：Linux/macOS 下配置文件权限会被收紧为 `0600`（仅属主可读写），因为其中可能包含 OBS 密码等敏感信息。
* **损坏处理**：若配置文件已损坏，为避免静默丢失其他设置，保存操作会被拒绝（不会用新内容覆盖修复）。遇到此情况请手动修复或删除该文件后重启。
